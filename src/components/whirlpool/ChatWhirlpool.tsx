...
interface ChatWhirlpoolProps {
  layers: WhirlpoolLayer[];
  assessmentData: AssessmentData;
}

const ChatWhirlpool: React.FC<ChatWhirlpoolProps> = ({ layers, assessmentData }) => {
  const [selectedLayer, setSelectedLayer] = useState<WhirlpoolLayer | null>(null);
  const [isHovered, setIsHovered] = useState(false);
  const [sortedLayers, setSortedLayers] = useState<WhirlpoolLayer[]>([]);
  
  // Sort layers by size (largest first) to ensure proper stacking
  useEffect(() => {
    const sorted = [...layers].sort((a, b) => b.score - a.score);
    setSortedLayers(sorted);
  }, [layers]);

  // Calculate the size of each layer based on assessment data
  const getLayerSize = (layer: WhirlpoolLayer) => {
    // Use the score directly from the layer object (already populated from assessment data)
    // This creates sizes between 50% and 100% based on the score
    return 50 + (layer.score * 50); // Base size of 50% plus up to 50% based on score
  };

  // Generate insights based on assessment data
  const generateInsights = (layer: WhirlpoolLayer) => {
    const score = layer.score * 100; // Convert decimal back to percentage
    const insights = [];
  ...
  return (
    <div className="relative">
      <div 
        className="relative w-full aspect-square border border-gray-100 rounded-full overflow-hidden"
